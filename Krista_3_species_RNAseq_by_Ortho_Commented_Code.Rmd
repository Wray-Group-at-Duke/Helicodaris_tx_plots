...
write_tsv(Ht_rna_counts, "Ht_rna_raw_counts.txt") 
...
write_tsv(Ht_orth_rna_counts, "Ht_orth_rna_counts.txt") 
...
Htub_sample_info <- read.csv("Ht_sample_info.csv")
...
HT_processed <- named_rlog_rna_count_means %>% select(names, starts_with("HT")) %>% mutate(species = "HT")
...
names(HT_processed) <- c("orthogroup", 0, 2, 3, 4, 10, 16, 24, "species")
...
HT_long <- HT_processed %>% pivot_longer(cols = c("0", "2", "3", "4", "10", "16", "24"), names_to = "hpf", values_to = "ex")
HE_long <- HE_processed %>% pivot_longer(cols = c("0", "2", "3", "4", "10", "16", "24"), names_to = "hpf", values_to = "ex")

# Re-combines the three species
combined_processed_data <- rbind(LV_long, HT_long, HE_long)

# Saves the final data table
write_tsv(combined_processed_data, "Final_Long_Lv_Ht_He.txt") 
...
# Change to desired orthogroup name
...
combined_processed_data |>
  ...
  ggplot(aes(hpf, ex, color=species)) +
        geom_line(size = 1.25) +
        labs(y = NULL) +
        labs(title = str_c(input_g1, ' Expression')) +
        theme_minimal() +
  scale_x_continuous(breaks=c(0, 2, 3, 4, 10, 16, 24)) +
scale_color_manual(values=c("#148040", "#148040", "#FAB120")) +
        theme(
